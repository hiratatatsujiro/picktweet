...
RSpec.describe "ツイート削除", type: :system do
  ...
  context "ツイート削除ができるとき" do
    it "ログインしたユーザーは自分が投稿したツイートを削除できる" do
      # ツイート１を投稿したユーザーでログインする
      sign_in(@tweet1.user)
      # ツイート１に削除ボタンがあることを確認する
      # binding.pry
      expect(
        all('.more')[1].hover
  ...
  context "ツイート削除ができないとき" do
    ...
    it "ログインしていないとツイートの削除ボタンがないことを確認する" do
      # トップページに移動する
      visit root_path
